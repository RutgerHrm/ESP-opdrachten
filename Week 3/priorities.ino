...
static QueueHandle_t opdrqueue;
...
static void press_task(void *argp) {
  ...
  for (;;) {
    ...
    if ( state == enable ) {
      ...
      if( xQueueSendToBack(opdrqueue, printLocalTime(), 0) != pdPASS)
      {
        xQueueReceive(opdrqueue, &prior, 0);
        printf("Data van %s vrij na 10 presses\r\n", prior);
        xQueueSendToBack(opdrqueue, printLocalTime(), 0);
      }
...
static void timeprio(void * argp){
...
void setup() {
  ...
  assert(queue);
  opdrqueue = xQueueCreate(10,30*sizeof(char));
  assert(opdrqueue); 
